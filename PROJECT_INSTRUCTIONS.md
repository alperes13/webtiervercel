...
- **Görevler:**
  ...
  - API entegrasyonları
  - E-posta OTP akışı implementasyonu
...
- **Görevler:**
  ...
  - XSS/CSRF koruması
  - Rate limiting kontrolü (E-posta OTP için kritik)
...
[Kullanıcı site URL girer] 
    ...
    → [E-posta adresi input modal açılır]
    → [E-posta adresi girer]
    → [GET isteği → dış sisteme e-posta adresi gönderilir]
    → [Dış sistem e-posta ile OTP kodu gönderir]
...
const OTP_CONFIG = {
  ...
  emailFormat: /^[^\s@]+@[^\s@]+\.[^\s@]+$/, // E-posta formatı
};
```
...
const ENDPOINTS = {
  ...
  sendOTP: '/api/v1/otp/send?email={email}&site={siteUrl}',
  
  // OTP doğrula
  verifyOTP: '/api/v1/otp/verify?email={email}&code={code}',
...
interface UserSession {
  token: string;
  email: string;           // Maskelenmiş: a***@domain.com
  emailRaw: string;        // Tam e-posta (hidden, "göster" butonu ile)

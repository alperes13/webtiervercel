...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f8fafc;padding:40px 0;margin:0;">
  <div style="max-width:420px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;">
    ...
    <div style="padding:32px 24px;text-align:center;">
      <p style="color:#334155;font-size:15px;margin:0 0 8px;font-weight:600;">Şifre Sıfırlama</p>
      <p style="color:#64748b;font-size:13px;margin:0 0 24px;">Aşağıdaki butona tıklayarak yeni şifrenizi belirleyebilirsiniz.</p>
      <a href="${resetUrl}" style="display:inline-block;background:#e11d48;color:#fff;text-decoration:none;padding:14px 32px;border-radius:8px;font-weight:700;font-size:14px;letter-spacing:0.5px;">Şifremi Sıfırla</a>
      <p style="color:#94a3b8;font-size:12px;margin:24px 0 0;">Bu link <strong>1 saat</strong> içinde geçerliliğini yitirecektir.</p>
      <p style="color:#94a3b8;font-size:11px;margin:8px 0 0;word-break:break-all;">${resetUrl}</p>
    </div>
    <div style="background:#f8fafc;padding:16px 24px;text-align:center;border-top:1px solid #e2e8f0;">
      <p style="color:#94a3b8;font-size:11px;margin:0;">Bu e-postayı siz talep etmediyseniz lütfen dikkate almayın.</p>
    </div>
...
export async function sendPasswordResetEmail(to: string, resetUrl: string): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    const { data, error } = await resend.emails.send({
      from,
      to: [to],
      subject: 'Webtier Şifre Sıfırlama',
